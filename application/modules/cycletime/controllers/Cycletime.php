...
class Cycletime extends Admin_Controller
{
  ...
  public function add()
  {
    ...
    $product    = $this->Cycletime_model->get_data_where_array('ms_product_category3');
  ...
  public function get_add()
  {
    ...
    $d_Header .= "&nbsp;<button type='button' class='btn btn-sm btn-danger delPart' title='Delete Part'><i class='fa fa-trash'></i></button>";
    $d_Header .= "</td>";
    $d_Header .= "</tr>";
  ...
  public function get_add_sub()
  {
    ...
    // $machine  = $this->db->query("SELECT * FROM asset WHERE category='12' AND deleted = 'N' GROUP BY SUBSTR(kd_asset, 1, 20) ORDER BY nm_asset ASC ")->result_array();
    $machine  = $this->db->query("SELECT * FROM asset WHERE category='12' AND deleted = 'N' GROUP BY kd_asset, nm_asset ORDER BY nm_asset ASC ")->result_array();
    // $mould  = $this->db->query("SELECT * FROM asset WHERE category='7' AND deleted = 'N' GROUP BY SUBSTR(kd_asset, 1, 20) ORDER BY nm_asset ASC ")->result_array();
    $mould  = $this->db->query("SELECT * FROM asset WHERE category='7' AND deleted = 'N' GROUP BY kd_asset, nm_asset ORDER BY nm_asset ASC ")->result_array();
    ...
    // $d_Header .= "<select name='Detail[".$id."][detail][".$no."][process]' class='chosen-select form-control input-sm inline-blockd process'>";
    ...
    // }
    // $d_Header .= 		"</select>";
    $d_Header .= "<input type='text' name='Detail[" . $id . "][detail][" . $no . "][process]' class='form-control input-md' placeholder='Process Name'  style='margin-bottom:15px;'>";
    ...
    foreach ($machine as $val => $valx) {
      $d_Header .= "<option value='" . $valx['id'] . "'>" . strtoupper($valx['nm_asset']) . "</option>";
    }
  ...
  public function excel_report()
  {
    ...
    $cycletime    = $this->db->query("SELECT 
                                      ...
                                      FROM 
                                        ...
                                        LEFT JOIN asset b ON a.id_product = b.id
