        ...
        <div class="col-md-10">
          ...
          <select id="produk" name="produk" class="form-control input-md chosen-select" disabled>
            <?php foreach ($results['product'] as $product) {
              $selected = ($product->id_category3 == $header[0]->id_product) ? 'selected' : '';
            ?>
              <option value="<?= $product->id_category3; ?>" <?= $selected; ?>><?= strtoupper(strtolower($product->nama)) ?></option>
            <?php } ?>
          </select>
        </div>
          ...
          <table class='table table-striped table-bordered table-hover table-condensed' width='100%'>
            ...
            <tbody>
              ...
              foreach ($q_header_test as $val2 => $val2x) {
                ...
                echo "&nbsp;<button type='button' class='btn btn-sm btn-danger delPart' title='Delete Part'><i class='fa fa-trash'></i></button>";
                echo "</td>";
                echo "</tr>";
                ...
                foreach ($q_dheader_test as $val2D => $val2Dx) {
                  ...
                  foreach ($mesin as $val => $valx) {
                    $sel = ($valx['id'] == $val2Dx['machine']) ? 'selected' : '';
                    echo "<option value='" . $valx['id'] . "' " . $sel . ">" . strtoupper($valx['nm_asset']) . "</option>";
                  }
                  echo     "</select>";
