...
class Lhp_mixing extends Admin_Controller
{
    protected $viewPermission     = 'LHP_Mixing.View';
    protected $addPermission      = 'LHP_Mixing.Add';
    protected $managePermission = 'LHP_Mixing.Manage';
    protected $deletePermission = 'LHP_Mixing.Delete';
    public function __construct()
    {
        ...
        $this->load->model(array(
            'Lhp_mixing/Lhp_mixing_model',
            'Aktifitas/aktifitas_model',
        ));
        $this->load->helper(['url', 'json']);
        $this->template->title('Lhp_mixing');
    ...
    public function save_data()
    {
        ...
        $get_material_tambahan = $this->db->where(['a.id_spk' => $id_spk, 'a.id_proses' => 5]);
    ...
    public function print_checksheet($id_spk)
    {
        ...
        $get_bom_detail = $this->db->query("
            ...
            FROM 
                ...
                LEFT JOIN ms_lhp_aktual_qty d ON d.id_spk = '" . $id_spk . "' AND d.id_material = a.id_category1 AND d.id_proses = '5'
            WHERE 
        ...
        $closing_spk = 0;
        $check_closing_spk = $this->db->get_where('ms_closing_lhp', ['id_spk' => $id_spk, 'id_so' => $get_data_spk->id_so, 'id_proses' => 5])->num_rows();
    ...
    public function getData()
    {
        ...
        $sql = "
            ...
            WHERE
                1=1 AND d.id_proses = '5' AND (SELECT COUNT(aa.id_spk) AS count_spk FROM ms_create_spk aa WHERE aa.id_so = a.id_so AND aa.id_proses = '5') > 0 AND (
    ...
    public function delete()
    {
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            $return    = array(
                'msg'        => "Failed delete data Lhp_mixing. Please try again. " . $errMsg,
                'status'    => 0
            );
        } else {
            $this->db->trans_commit();
            $return    = array(
                'msg'        => 'Delete data Lhp_mixing.',
                'status'    => 5
            );
    ...
    public function list_spk($id_so)
    {
        ...
        $list_data = $this->db->where(['a.id_so' => str_replace('-', '/', $id_so), 'a.id_proses' => 5]);
