			...
			$('#save').click(function(e) {
				...
				swal.fire({
					...
					if (hasil.isConfirmed) {
						...
						var baseurl = siteurl + 'app_pr_stock/process_approval_all';
						$.ajax({
							url: baseurl,
							type: "POST",
							data: formData,
							...
							success: function(data) {
								if (data.status == 1) {
									...
									window.location.href = base_url + 'app_pr_stock'
								} else {
									swal.fire({
										title: "Save Failed!",
										text: data.pesan,
			...
			$(document).on('click', '.processSatuan', function(e) {
				...
				swal.fire({
					...
					if (hasil.isConfirmed) {
						$.ajax({
							type: 'POST',
							url: base_url + 'app_pr_stock/process_approval_satuan',
							dataType: "json",
							data: {
								'id': id,
								'action': action,
							...
							success: function(result) {
								if (result.status == '1') {
									...
									window.location.href = base_url + 'app_pr_stock/approval_planning/' + result.so_number
								} else {
									swal.fire({
										title: "Error",
										text: result.pesan,
