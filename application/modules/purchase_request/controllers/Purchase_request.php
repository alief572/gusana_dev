...
class Purchase_request extends Admin_Controller
{
	...
	public function index($status_filter = null)
	{
		...
		$this->template->title('List Outstanding PR');
	...
	public function View($id)
	{
		...
		$session = $this->session->userdata('app_session');
		$this->template->page_icon('fa fa-pencil');
		$header = $this->db->query("SELECT a.*, c.nm_customer, b.full_name AS nama_user FROM material_planning_base_on_produksi a LEFT JOIN users b ON b.id_user = a.booking_by LEFT JOIN customer c ON c.id_customer = a.id_customer WHERE a.so_number = '$id' ")->result();
		$detail     = $this->db
			->select('a.*, b.nama as nm_product, b.code as code_material, b.konversi as qty_pack, b.max_stok, b.min_stok, c.code as satuan_packing, d.code as unit_measure')
	...
	public function View_depart($id)
	{
		...
		$header 	= $this->db->query("SELECT a.*, b.full_name as nm_user FROM rutin_non_planning_header a LEFT JOIN users b ON b.id_user = a.created_by WHERE a.no_pr = '" . $id . "' ")->result();
	...
	public function PrintH2()
	{
		...
		// $this->auth->restrict($this->managePermission);
		$id = $this->uri->segment(3);
		$data['header'] = $this->db->query("SELECT a.*, b.customer_name, b.address as alamat FROM material_planning_base_on_produksi as a LEFT JOIN material_planning_base_on_produksi x ON x.so_number = a.so_number LEFT JOIN customers b ON b.id_customer = a.id_customer  WHERE a.so_number = '" . $id . "' ")->result();
		...
		$data['detail']  = $this->db->query("
			...
			FROM 
				material_planning_base_on_produksi_detail a 
			LEFT JOIN 
				ms_inventory_category3 b ON b.id_category3 = a.id_material 
			LEFT JOIN 
				ms_satuan c ON c.id = b.unit
			LEFT JOIN 
				ms_satuan d ON d.id = b.packaging
			WHERE a.so_number = '" . $id . "' 
		")->result();
