...
class Man_power_rate_model extends BF_Model{
	...
	public function get_data_komp_by_id($id){
		$getData = $this->db->get_where('ms_komp_man_power_rate',['id' => $id])->row();

		return $getData;
	}

	public function get_choose_komp_by_tipe($tipe){
		$getData = $this->db->get_where('ms_choose_komp_man_power_rate',['tipe' => $tipe])->result();
