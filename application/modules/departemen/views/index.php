<?php
$ENABLE_ADD     = has_permission('Departemen.Add');
$ENABLE_MANAGE  = has_permission('Departemen.Manage');
$ENABLE_VIEW    = has_permission('Departemen.View');
$ENABLE_DELETE  = has_permission('Departemen.Delete');
?>
...
<div class="d-flex align-items-center justify-content-between pd-x-20 pd-sm-x-30 pd-t-25 mg-b-20 mg-sm-b-30">
    ...
    <?php if ($ENABLE_ADD) : ?>
        <button class="btn btn-primary btn-oblong add" href="javascript:void(0)" title="Add"><i class="fa fa-plus">&nbsp;</i>Add New Departemen</button>
            ...
            <table id="dataTable" width="100%" class="table display table-bordered table-hover table-striped border-left-0 border-right-0">
                <thead>
                    <tr>
                        ...
                        <th class="desktop tablet mobile tx-bold tx-dark" width="">Departemen</th>
...
<script type="text/javascript">
    ...
    $(document).on('click', '.add', function() {
        $.ajax({
            ...
            success: function(data) {
                $('#dialog-popup .modal-title').html("<span class='<?= $template['page_icon']; ?>'></span>&nbsp;Add New Departemen")
    ...
    $(document).on('click', '.edit', function(e) {
        ...
        $.ajax({
            ...
            success: function(data) {
                $('#dialog-popup .modal-title').html("<span class='<?= $template['page_icon']; ?>'></span>&nbsp;Edit Departemen")
                $('#dialog-popup .modal-dialog').css({
                    'max-width': '90%'
                })
                $("#dialog-popup").modal();
