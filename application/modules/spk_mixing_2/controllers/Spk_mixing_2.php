...
/*
 * @author Hikmat Aolia
 * @copyright Copyright (c) 6063, Hikmat Aolia
 *
 * This is controller for Master Employee
 */

class Spk_mixing_2 extends Admin_Controller
{
    protected $viewPermission     = 'Mixing_2.View';
    protected $addPermission      = 'Mixing_2.Add';
    protected $managePermission = 'Mixing_2.Manage';
    protected $deletePermission = 'Mixing_2.Delete';
    public function __construct()
    {
        ...
        $this->load->model(array(
            'Spk_mixing_2/Spk_mixing_2_model',
            'Aktifitas/aktifitas_model',
        ));
        $this->load->helper(['url', 'json']);
        $this->template->title('Spk_mixing_2');
    ...
    public function create_spk()
    {
        ...
        $code_spk = $this->db->query("SELECT MAX(id_spk) as max_id_spk FROM ms_create_spk WHERE id_spk LIKE '%SPK-M-" . date('Ymd') . "-%' AND id_proses = '6'")->row();
        $kodeBarang = $code_spk->max_id_spk;
        $urutan = (int) substr($kodeBarang, 16, 6);
        $urutan++;
        $tahun = date('Ymd');
        $huruf = "SPK-M-";
        $kode_spk = $huruf . $tahun . '-' . sprintf("%06s", $urutan);

        $get_max_batch_ke = $this->db->query("SELECT MAX(batch_ke) AS max_batch_ke FROM ms_create_spk WHERE id_so = '" . $id_so . "' AND id_proses = '6'")->row();
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            $return    = array(
                'msg'        => 'Maaf, SPK Mixing_2 gagal dibuat',
                'status'    => 0,
                'id_spk' => $kode_spk
            );
        } else {
            ...
            $return    = array(
                'msg'        => 'Selamat, SPK Mixing_2 telah berhasil terbit',
                'status'    => 1,
                'id_spk'    => $kode_spk
            );
        }
    ...
    public function add_material_tambahan()
    {
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            $get_material_tambahan = $this->db->get_where('ms_spk_material_tambahan', ['id_spk' => $id_spk, 'id_proses' => '6'])->result();
    ...
    public function save_data()
    {
        ...
        $get_product = $this->db->where(['a.id_spk' => $id_spk, 'c.id_proses' => 6]);
        ...
        foreach ($get_product as $material) {
            ...
            if ($weight_input !== $material->weight) {
                $get_custom_weight = $this->db->get_where('ms_spk_weight_custom', ['id_spk' => $id_spk, 'id_proses' => 6, 'id_material' => $material->id_category1])->num_rows();
                if ($get_custom_weight > 0) {
                    $this->db->delete('ms_spk_weight_custom', ['id_spk' => $id_spk, 'id_proses' => 6, 'id_material' => $material->id_category1]);
    ...
    {
        $get_data_spk = $this->db->get_where('ms_create_spk', ['id_spk' => $id_spk, 'id_proses' => 6])->row();
        ...
        $get_bom_detail = $this->db->query("
            ...
            FROM 
                ...
                LEFT JOIN ms_spk_weight_custom c ON c.id_material = a.id_category1 AND c.id_spk = '" . $id_spk . "' AND c.id_proses = '6' 
            WHERE 
        ...
        $this->template->title('Print Checksheet Mixing 2');
    ...
    public function print_checksheet_real($id_spk)
    {
        ...
        $this->auth->restrict($this->viewPermission);
        $this->template->set('results', $data);
        $this->template->title('Print Checksheet Mixing_2');
    ...
    public function getData()
    {
        ...
        foreach ($query->result_array() as $row) {
            ...
            if (
                ...
                $nomor = ($total_data - $start_dari) - $urut6;
            }

            $num_create_spk = $this->db->query('SELECT IF(batch_ke IS NOT NULL, MAX(batch_ke), 0) AS max_batch_ke FROM ms_create_spk WHERE id_so = "' . $row['id_so'] . '" AND id_proses = "6"')->num_rows();

            $get_create_spk = $this->db->query('SELECT IF(batch_ke IS NOT NULL, MAX(batch_ke), 0) AS max_batch_ke FROM ms_create_spk WHERE id_so = "' . $row['id_so'] . '" AND id_proses = "6"')->row();
            ...
            $create_spk_btn = '<button type="button" class="btn btn-primary btn-sm create_spk" data-id_so="' . $row['id_so'] . '" data-id_proses="6" data-batch_ke="' . ($get_create_spk->max_batch_ke + 1) . '">Create SPK</button>';
            if (($get_create_spk->max_batch_ke) == (($row['propose'] / $get_bom->qty_hopper))) {
                $create_spk_btn = '';
            }
            $btn_list_spk = '';
            if ($num_create_spk > 0) {
                $btn_list_spk = '<button type="button" class="btn btn-info btn-sm list_spk" data-id_so="' . str_replace("/", "-", $row['id_so']) . '" data-id_proses="6" data-batch_ke="' . ($get_create_spk->max_batch_ke + 1) . '">List SPK</button>';
            }
    ...
    public function delete()
    {
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            $return    = array(
                'msg'        => "Failed delete data Spk_mixing_2. Please try again. " . $errMsg,
                'status'    => 0
            );
        } else {
            $this->db->trans_commit();
            $return    = array(
                'msg'        => 'Delete data Spk_mixing_2.',
                'status'    => 1
            );
    ...
    public function list_spk($id_so)
    {
        ...
        $list_data = $this->db->where(['id_so' => str_replace('-', '/', $id_so), 'id_proses' => 6]);
