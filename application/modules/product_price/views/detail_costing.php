...
foreach ($detail as $val => $valx) {
    ...
    $price_ref      = (!empty($GET_PRICE_REF[$valx['id']]['price_ref_idr'])) ? $GET_PRICE_REF[$valx['id']]['price_ref_idr'] : 0;
                ...
                <form action="" method="post" id="data-form">
                    ...
                    foreach ($dataList as $key => $value) {
                        if ($value['judul'] == 'Manpower') {
                            if ($value['code'] == '2') {
                                $rate         = number_format($product_price[0]['rate_cycletime'], 2) . ' x ' . number_format($product_price[0]['rate_man_power_idr'], 2);
                                $man_power    = $product_price[0]['cost_direct_labout'];
                                $detRate = "<span class='text-primary btncursor detailRate' id='btnShowMold' data-tanda='manpower' data-cost='" . $product_price[0]['rate_man_power_idr'] . "' data-id_product='" . $header[0]->id_product . "' >Detail</span>";
                            }
                            if ($value['code'] == '3') {
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $(document).on('click', '#btnShowMaterial', function() {
                ...
                $.ajax({
                    type: 'POST',
                    url: base_url + thisController + 'detail_material',
                    data: {
                        'no_bom': no_bom
                    },
            ...
            $(document).on('click', '.detailRate', function() {
                ...
                $.ajax({
                    type: 'POST',
                    url: base_url + thisController + 'detail_machine_mold',
                    data: {
