...
class Spk_penimbangan extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model(array(
            'Spk_penimbangan/Spk_penimbangan_model',
            'Aktifitas/aktifitas_model',
        ));
        $this->load->helper(['url', 'json']);
        $this->template->title('Spk_penimbangan');
    ...
    public function save_data()
    {
        ...
        foreach ($get_product as $material) {
            ...
            if ($weight_input !== $material->weight) {
                $get_custom_weight = $this->db->get_where('ms_spk_weight_custom', ['id_spk' => $id_spk, 'id_proses' => 1, 'id_material' => $material->id_category1])->num_rows();
                if ($get_custom_weight > 0) {
                    $this->db->delete('ms_spk_weight_custom', ['id_spk' => $id_spk, 'id_proses' => 1, 'id_material' => $material->id_category1]);
    ...
    public function delete()
    {
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            $return    = array(
                'msg'        => "Failed delete data Spk_penimbangan. Please try again. " . $errMsg,
                'status'    => 0
            );
        } else {
            $this->db->trans_commit();
            $return    = array(
                'msg'        => 'Delete data Spk_penimbangan.',
                'status'    => 1
            );
    ...
    public function list_spk($id_so)
    {
        ...
        $list_data = $this->db->where(['id_so' => str_replace('-', '/', $id_so), 'id_proses' => 1]);
