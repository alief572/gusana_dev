...
class Warehouse_material_model extends BF_Model{
  ...
  public function get_json_adjustment(){
    ...
    foreach($query->result_array() as $row){
      ...
      $nestedData[]	= "<div align='left'>".strtoupper(get_name('m_warehouse', 'warehouse_nm', 'id', $row['kd_gudang']))."</div>";
  ...
  public function get_query_json_adjustment($material, $kd_gudang, $like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL){
    ...
    $sql = "
            ...
            FROM
              ...
              LEFT JOIN m_warehouse b ON a.kd_gudang = b.id
              LEFT JOIN ms_inventory_category3 c ON a.material = c.id_category3,
              (SELECT @row:=0) r
            WHERE 1=1 ".$material_where." ".$kd_gudang_where." AND (
              b.warehouse_nm LIKE '%".$this->db->escape_like_str($like_value)."%'
