...
<script type="text/javascript">
    ...
    $(document).on('click', '.view', function() {
        ...
        $.ajax({
            ...
            beforeSend: function(result) {
                ...
                $('#myModalLabel').html('Detail Request Material');
            },
    ...
    $(document).on('click', '.print', function() {
        ...
        swalWithBootstrapButtons.fire({
            title: "Are you sure?",
            text: "printing this request means you approved it !",
            icon: "warning",
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: 'post',
                    url: siteurl + thisController + 'print_outgoing',
                    data: {
                        'id': id
                    },
                    cache: false,
                    ...
                    success: function(result) {
                        if (result.status == '1') {
                            window.open(siteurl + thisController + 'print_ttb/' + id);
                            location.reload(true);
                        } else {
                            if (result.status == '2') {
                                Swal.fire("Failed !", "This request can't be done because lack of stock !", 'error');
                            } else {
                                Swal.fire("Failed !", "Updating status process is failed !", 'error');
                            }
                        }
                    }
                });
