...
class Warehouse_material extends Admin_Controller
{
  ...
  public function adjustment_material()
  {
    ...
    $this->template->title('Adjustment Material');
    $this->template->page_icon('fa fa-building-o');
  ...
  public function get_stock($material)
  {
    ...
    if (empty($unit)) {
      $stock = get_stock_material_packing($material);
    }
    if (!empty($unit)) {
      if ($IMP[1] == 'unit') {
    ...
    $sqlSup    = "SELECT b.nm_packaging, c.nm_unit FROM ms_inventory_category3 a LEFT JOIN master_packaging b ON b.id = a.packaging LEFT JOIN m_unit c ON c.id_unit = a.unit WHERE a.id_category3 ='" . $material . "' ";
