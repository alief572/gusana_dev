...
<div class="box">
	<div class="box-header">
		<?php if ($ENABLE_ADD) : ?>
			...
			<a class="btn btn-success btn-md" style='float:right;' href="<?= base_url('request_pr_stok/add') ?>" title="Add">Add</a>
		<?php endif; ?>
		<br>
...
<div class="modal modal-default fade" id="dialog-popup" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	...
	<script type="text/javascript">
		$(document).on('click', '.detail', function() {
			...
			$.ajax({
				type: 'POST',
				url: base_url + thisController + 'detail',
				data: {
					'so_number': so_number,
				},
		...
		$(document).on('click', '.booking', function(e) {
			...
			swal({
				...
				function() {
					$.ajax({
						type: 'POST',
						url: base_url + thisController + 'process_booking',
						dataType: "json",
						data: {
							'so_number': so_number
						},
		...
		function DataTables(costcenter = null, product = null) {
			var dataTable = $('#example1').DataTable({
				...
				"ajax": {
					url: siteurl + thisController + 'data_side_approval_pr_material',
					type: "post",
					data: function(d) {
