...
class Spk_filling extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model(array(
            'Spk_filling/Spk_filling_model',
            'Aktifitas/aktifitas_model',
        ));
        $this->load->helper(['url', 'json']);
        $this->template->title('Spk_filling');
    ...
    public function create_spk()
    {
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            $return    = array(
                'msg'        => 'Maaf, SPK Filling gagal dibuat',
                'status'    => 0,
                'id_spk' => $kode_spk
            );
        } else {
            ...
            $return    = array(
                'msg'        => 'Selamat, SPK Filling telah berhasil terbit',
                'status'    => 1,
                'id_spk'    => $kode_spk
            );
        }
    ...
    public function save_data()
    {
        ...
        $get_product = $this->db->where(['a.id_spk' => $id_spk, 'c.id_proses' => '-']);
        ...
        foreach ($get_product as $material) {
            ...
            if ($weight_input !== $material->weight) {
                $get_custom_weight = $this->db->get_where('ms_spk_weight_custom', ['id_spk' => $id_spk, 'id_proses' => '-', 'id_material' => $material->id_category1])->num_rows();
                if ($get_custom_weight > 0) {
                    $this->db->delete('ms_spk_weight_custom', ['id_spk' => $id_spk, 'id_proses' => '-', 'id_material' => $material->id_category1]);
    ...
    public function getData()
    {
        ...
        foreach ($query->result_array() as $row) {
            ...
            $num_create_spk = $this->db->query('SELECT IF(batch_ke IS NOT NULL, MAX(batch_ke), 0) AS max_batch_ke FROM ms_create_spk WHERE id_so = "' . $row['id_so'] . '" AND id_proses = "-"')->num_rows();

            $get_create_spk = $this->db->query('SELECT IF(batch_ke IS NOT NULL, MAX(batch_ke), 0) AS max_batch_ke FROM ms_create_spk WHERE id_so = "' . $row['id_so'] . '" AND id_proses = "-"')->row();
            ...
            $btn_list_spk = '';
            if ($num_create_spk > 0) {
                $btn_list_spk = '<button type="button" class="btn btn-info btn-sm list_spk" data-id_so="' . str_replace("/", "-", $row['id_so']) . '" data-id_proses="-" data-batch_ke="' . ($get_create_spk->max_batch_ke + 1) . '">List SPK</button>';
            }
    ...
    public function delete()
    {
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            $return    = array(
                'msg'        => "Failed delete data Spk_filling. Please try again. " . $errMsg,
                'status'    => 0
            );
        } else {
            $this->db->trans_commit();
            $return    = array(
                'msg'        => 'Delete data Spk_filling.',
                'status'    => 1
            );
