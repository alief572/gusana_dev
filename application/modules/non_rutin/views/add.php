	...
	<div class="box box-primary">
		...
		<div class="box-body">
			<div class='form-group row'>
				...
				<div class='col-sm-4'>
					<select name='id_dept' id='id_dept' class='form-control input-md chosen_select' <?= $disabled; ?>>
						...
						foreach ($list_divisi as $divisi) {
							$selected = '';
							if ($divisi->id == $id_dept) {
								$selected = 'selected';
							}
							echo "<option value='" . $divisi->id . "' " . $selected . ">" . strtoupper($divisi->divisi) . "</option>";
						}
						?>
					</select>
				</div>
				...
				<div class='col-sm-4'>
					<input type="text" name="project_name" id="project_name" class="form-control input-md" value="<?= $project_name ?>" placeholder="Project Name" <?= $disabled; ?>>
			...
			<div class='form-group row'>
				...
				<div class='col-sm-4  text-right'>
					...
					<?php if (!empty($upload_spk)) { ?>
						<a href='<?= base_url('uploads/pr_departemen/' . $upload_spk); ?>' target='_blank' title='Download' data-role='qtip'>Download</a>
					<?php } ?>
				</div>
			</div>
				...
				<tbody>
					...
					if (!empty($detail)) {
						foreach ($detail as $val => $valx) {
							...
							echo "<td align='left'><input type='text' " . $disabled2 . " id='harga_" . $nomor . "' name='detail[" . $nomor . "][harga]' class='form-control input-md text-right maskM sum_tot' value='" . $valx['harga'] . "' ></td>";
							...
							if (empty($approve)) {
								echo "<td align='center'><button type='button' class='btn btn-sm btn-danger delPart' title='Delete Part'>Delete</button></td>";
							}
							echo "</tr>";
						}
					}
...
<script>
	...
	$(document).on('click', '#back', function(e) {
		...
		window.location.href = base_url + thisController + tanda;
	});
	...
	$(document).on('click', '.addPart', function() {
		...
		$.ajax({
			url: base_url + thisController + 'get_add/' + id,
			cache: false,
			type: "POST",
			dataType: "json",
	...
	$(document).on('click', '#save', function(e) {
		...
		swal.fire({
				...
				if (hasil.isConfirmed) {
					...
					var baseurl = base_url + thisController + 'add';
					$.ajax({
						url: baseurl,
						type: "POST",
						data: formData,
	...
	function sum_total(a) {
		var qty = parseFloat($('#qty_' + a).val().split(",").join(""));
		var harga = parseFloat($('#harga_' + a).val().split(",").join(""));
