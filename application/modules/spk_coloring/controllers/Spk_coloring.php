...
class Spk_coloring extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model(array(
            'Spk_coloring/Spk_coloring_model',
            'Aktifitas/aktifitas_model',
        ));
        $this->load->helper(['url', 'json']);
        $this->template->title('Spk_coloring');
    ...
    public function create_spk()
    {
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            $return    = array(
                'msg'        => 'Maaf, SPK Coloring gagal dibuat',
                'status'    => 0,
                'id_spk' => $kode_spk
            );
        } else {
            ...
            $return    = array(
                'msg'        => 'Selamat, SPK Coloring telah berhasil terbit',
                'status'    => 1,
                'id_spk'    => $kode_spk
            );
        }
    ...
    public function save_data()
    {
        ...
        foreach ($get_product as $material) {
            ...
            if ($weight_input !== $material->weight) {
                $get_custom_weight = $this->db->get_where('ms_spk_weight_custom', ['id_spk' => $id_spk, 'id_proses' => 2, 'id_material' => $material->id_category1])->num_rows();
                if ($get_custom_weight > 0) {
                    $this->db->delete('ms_spk_weight_custom', ['id_spk' => $id_spk, 'id_proses' => 2, 'id_material' => $material->id_category1]);
    ...
    public function print_checksheet($id_spk)
    {
        ...
        $this->template->title('Print Checksheet Coloring');
    ...
    public function getData()
    {
        ...
        foreach ($query->result_array() as $row) {
            ...
            $num_create_spk = $this->db->query('SELECT IF(batch_ke IS NOT NULL, MAX(batch_ke), 0) AS max_batch_ke FROM ms_create_spk WHERE id_so = "' . $row['id_so'] . '" AND id_proses = "2"')->num_rows();

            $get_create_spk = $this->db->query('SELECT IF(batch_ke IS NOT NULL, MAX(batch_ke), 0) AS max_batch_ke FROM ms_create_spk WHERE id_so = "' . $row['id_so'] . '" AND id_proses = "2"')->row();
            ...
            $create_spk_btn = '<button type="button" class="btn btn-primary btn-sm create_spk" data-id_so="' . $row['id_so'] . '" data-id_proses="2" data-batch_ke="' . ($get_create_spk->max_batch_ke + 1) . '">Create SPK</button>';
            if (($get_create_spk->max_batch_ke) == (($row['propose'] / $get_bom->qty_hopper))) {
                $create_spk_btn = '';
            }
            $btn_list_spk = '';
            if ($num_create_spk > 0) {
                $btn_list_spk = '<button type="button" class="btn btn-info btn-sm list_spk" data-id_so="' . str_replace("/", "-", $row['id_so']) . '" data-id_proses="2" data-batch_ke="' . ($get_create_spk->max_batch_ke + 1) . '">List SPK</button>';
            }
    ...
    public function delete()
    {
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            $return    = array(
                'msg'        => "Failed delete data Spk_coloring. Please try again. " . $errMsg,
                'status'    => 0
            );
        } else {
            $this->db->trans_commit();
            $return    = array(
                'msg'        => 'Delete data Spk_coloring.',
                'status'    => 1
            );
    ...
    public function list_spk($id_so)
    {
        ...
        $list_data = $this->db->where(['id_so' => str_replace('-', '/', $id_so), 'id_proses' => 2]);
