    ...
    <div class="box-body" style="padding: 10px;">
        <table id="dataTable" class="table table-bordered table-striped">
            ...
            <tbody>
                <?php
                $no = 1;
                foreach ($results['list_product_stock'] as $product_stock) :


                    echo '
						<tr>
							<td class="text-center">' . $no . '</td>
							<td class="text-center">' . $product_stock->category_nm . '</td>
							<td class="text-center">' . $product_stock->nama . '</td>
							<td class="text-center">' . $product_stock->nm_packaging . '</td>
							<td class="text-center">' . number_format($product_stock->konversi) . '</td>
							<td class="text-center">' . $product_stock->nm_unit . '</td>
							<td class="text-center">' . number_format($product_stock->stock_unit, 2) . '</td>
							<td class="text-center">
								<button type="button" class="btn btn-sm btn-info history_product history_product_' . $product_stock->id_category3 . '" data-id_category3="' . $product_stock->id_category3 . '"><i class="fa fa-eye"></i></button>
							</td>
						</tr>
					';
                    $no++;
...
<script type="text/javascript">
    ...
    $(document).on('click', '.history_product', function() {
        var id_category3 = $(this).data('id_category3');

        $.ajax({
            type: "POST",
            url: siteurl + active_controller + '/history_product',
            data: {
                'id_category3': id_category3
            },
            cache: false,
            beforeSend: function(result) {
                $('.history_product_' + id_category3).html('<i class="fa fa-spin fa-spinner"></i>');
            },
            success: function(result) {
                $('#myModalLabel').html('Data Stock Product');
                $('#ModalView').html(result);
                $('#dialog-popup').modal();

                $('.history_product_' + id_category3).html('<i class="fa fa-eye"></i>');
            },
