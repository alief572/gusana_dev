...
class Production_warehouse extends Admin_Controller
{
	...
	public function search_history()
	{
        ...
        FROM
			...
			LEFT JOIN ms_inventory_category3 f ON f.id_category1 = d.id_category1
        WHERE
             d.id_category1 = '" . $id_category1 . "' AND a.status = '1' AND a.id IS NOT NULL AND a.tgl_request BETWEEN '" . $post['from_tgl'] . "' AND '" . $post['to_tgl'] . "'
		...
		FROM
			...
			LEFT JOIN ms_inventory_category3 h ON h.id_category1 = c.id_category1
		WHERE
			c.id_category1 = '" . $id_category1 . "' AND d.sts = 'closing' AND d.id_spk IS NOT NULL AND DATE_FORMAT(d.dibuat_tgl, '%Y-%m-%d') >= '2024-02-26' AND DATE_FORMAT(a.due_date, '%Y-%m-%d') >= '2024-02-01' AND DATE_FORMAT(d.dibuat_tgl, '%Y-%m-%d') BETWEEN '" . $post['tgl_from'] . "' AND '" . $post['tgl_to'] . "'
			...
			if ($history->no_transaksi !== NULL) {
				...
				$hasil .= '
					<tr>
						...
						<td class="text-center">' . number_format($history->qty_down * $history->konversi) . '</td>
						<td class="text-center">' . number_format($history->qty_up * $history->konversi) . '</td>
