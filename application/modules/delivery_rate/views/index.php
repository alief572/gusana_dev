<?php
$ENABLE_ADD     = has_permission('Delivery_Rate.Add');
$ENABLE_MANAGE  = has_permission('Delivery_Rate.Manage');
$ENABLE_VIEW    = has_permission('Delivery_Rate.View');
$ENABLE_DELETE  = has_permission('Delivery_Rate.Delete');
?>
<style type="text/css">
	thead input {
		width: 100%;
	...
	<div class="modal modal-default fade" id="dialog-country" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		...
		<script type="text/javascript">
			...
			$(document).on('click', '#addCountry', function() {
				$.ajax({
					type: 'POST',
					url: siteurl + thisController + '/add_country',
					success: function(data) {
			...
			$(document).on('submit', '#data_form', function(e) {
				...
				if (type == 'local') {
					...
					if (category == '0') {
						new swal({
							title: "Error Message!",
							text: 'Delivery Category not selected...',
							type: "warning"
						});
						return false;
					}
					...
					if (shipping_method == '0') {
						new swal({
							title: "Error Message!",
							text: 'Shipping Method not selected...',
							type: "warning"
						});
						return false;
					}
