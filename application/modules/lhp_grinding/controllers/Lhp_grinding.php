...
class Lhp_grinding extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model(array(
            'Lhp_grinding/Lhp_grinding_model',
            'Aktifitas/aktifitas_model',
        ));
        $this->load->helper(['url', 'json']);
        $this->template->title('Lhp_grinding');
    ...
    public function save_data()
    {
        ...
        $get_material_tambahan = $this->db->where(['a.id_spk' => $id_spk, 'a.id_proses' => 3]);
    ...
    public function print_checksheet($id_spk)
    {
        ...
        $get_bom_detail = $this->db->query("
            ...
            FROM 
                ...
                LEFT JOIN ms_lhp_aktual_qty d ON d.id_spk = '" . $id_spk . "' AND d.id_material = a.id_category1 AND d.id_proses = '3'
            WHERE 
    ...
    public function getData()
    {
        ...
        $sql = "
            ...
            WHERE
                1=1 AND d.id_proses = '3' AND (SELECT COUNT(aa.id_spk) AS count_spk FROM ms_create_spk aa WHERE aa.id_so = a.id_so AND aa.id_proses = '3') > 0 AND (
    ...
    public function delete()
    {
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            $return    = array(
                'msg'        => "Failed delete data Lhp_grinding. Please try again. " . $errMsg,
                'status'    => 0
            );
        } else {
            $this->db->trans_commit();
            $return    = array(
                'msg'        => 'Delete data Lhp_grinding.',
                'status'    => 3
            );
    ...
    public function list_spk($id_so)
    {
        ...
        $list_data = $this->db->where(['a.id_so' => str_replace('-', '/', $id_so), 'a.id_proses' => 3]);
