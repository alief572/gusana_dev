...
class Lhp_penimbangan extends Admin_Controller
{
    protected $viewPermission     = 'LHP_Penimbangan.View';
    protected $addPermission      = 'LHP_Penimbangan.Add';
    protected $managePermission = 'LHP_Penimbangan.Manage';
    protected $deletePermission = 'LHP_Penimbangan.Delete';
    public function __construct()
    {
        ...
        $this->load->model(array(
            'Lhp_penimbangan/Lhp_penimbangan_model',
            'Aktifitas/aktifitas_model',
        ));
        $this->load->helper(['url', 'json']);
        $this->template->title('Lhp_penimbangan');
    ...
    public function create_spk()
    {
        ...
        $code_spk = $this->db->query("SELECT MAX(id_spk) as max_id_spk FROM ms_create_spk WHERE id_spk LIKE '%SPK-M-" . date('Ymd') . "-%' AND id_proses = '1'")->row();
        ...
        $get_max_batch_ke = $this->db->query("SELECT MAX(batch_ke) AS max_batch_ke FROM ms_create_spk WHERE id_so = '" . $id_so . "' AND id_proses = '1'")->row();
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            $return    = array(
                'msg'        => 'Maaf, SPK Penimbangan gagal dibuat',
                'status'    => 0,
                'id_spk' => $kode_spk
            );
        } else {
            ...
            $return    = array(
                'msg'        => 'Selamat, SPK Penimbangan telah berhasil terbit',
                'status'    => 1,
                'id_spk'    => $kode_spk
            );
        }
    ...
    public function add_material_tambahan()
    {
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            $get_material_tambahan = $this->db->get_where('ms_spk_material_tambahan', ['id_spk' => $id_spk, 'id_proses' => '1'])->result();
    ...
    public function save_data()
    {
        ...
        $get_product = $this->db->where(['a.id_spk' => $id_spk, 'c.id_proses' => 1]);
        ...
        $get_material_tambahan = $this->db->where(['a.id_spk' => $id_spk, 'a.id_proses' => 1]);
    ...
    {
        $get_data_spk = $this->db->get_where('ms_create_spk', ['id_spk' => $id_spk, 'id_proses' => 1])->row();
        ...
        $get_bom_detail = $this->db->query("
            ...
            FROM 
                ...
                LEFT JOIN ms_spk_weight_custom c ON c.id_material = a.id_category1 AND c.id_spk = '" . $id_spk . "' AND c.id_proses = '1' 
                LEFT JOIN ms_lhp_aktual_qty d ON d.id_spk = '" . $id_spk . "' AND d.id_material = a.id_category1 AND d.id_proses = '1'
            WHERE 
        ...
        $closing_spk = 0;
        $check_closing_spk = $this->db->get_where('ms_closing_lhp', ['id_spk' => $id_spk, 'id_so' => $get_data_spk->id_so, 'id_proses' => 1])->num_rows();
        ...
        $this->template->title('Print Checksheet Penimbangan');
    ...
    public function getData()
    {
        ...
        $sql = "
            ...
            WHERE
                1=1 AND d.id_proses = '1' AND (SELECT COUNT(aa.id_spk) AS count_spk FROM ms_create_spk aa WHERE aa.id_so = a.id_so AND aa.id_proses = '1') > 0 AND (
    ...
    public function delete()
    {
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            $return    = array(
                'msg'        => "Failed delete data Lhp_penimbangan. Please try again. " . $errMsg,
                'status'    => 0
            );
        } else {
            $this->db->trans_commit();
            $return    = array(
                'msg'        => 'Delete data Lhp_penimbangan.',
                'status'    => 1
            );
    ...
    public function list_spk($id_so)
    {
        ...
        $list_data = $this->db->where(['a.id_so' => str_replace('-', '/', $id_so), 'a.id_proses' => 1]);
