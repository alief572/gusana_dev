...
{
    protected $viewPermission     = 'Divisi.View';
    protected $addPermission      = 'Divisi.Add';
    protected $managePermission = 'Divisi.Manage';
    protected $deletePermission = 'Divisi.Delete';
    ...
    public function getData()
    {
        ...
        $sql = "SELECT *
        FROM m_divisi WHERE 1=1  
        AND (divisi LIKE '%$string%')";
    ...
    public function save()
    {
        ...
        if ($num_divisi > 0) {
            ...
            $desc = "Update Divisi Data " . $data['id'] . " - " . $data['divisi'];
            $device_name = $this->agent->mobile(); // Returns the mobile device name
            if ($this->agent->is_browser()) {
                $device_name = $this->agent->browser(); // Returns the browser name
            } elseif ($this->agent->is_robot()) {
            ...
            $desc = "Insert New Divisi Data " . $data['id'] . " - " . $data['divisi'];
            $device_name = $this->agent->mobile(); // Returns the mobile device name
            if ($this->agent->is_browser()) {
                $device_name = $this->agent->browser(); // Returns the browser name
            } elseif ($this->agent->is_robot()) {
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            $keterangan     = "FAILED save data Divisi " . $data['id'] . ", Divisi : " . $data['divisi'];
            $status         = 1;
            ...
            $return    = array(
                'msg'        => 'Success Save data Divisi.',
                'status'    => 1
            );
            $keterangan     = "SUCCESS save data Divisi " . $data['id'] . ", Divisi : " . $data['divisi'];
            $status         = 1;
            $nm_hak_akses   = $this->addPermission;
            $kode_universal = $data['id'];
            $jumlah         = 1;
