...
class Unit extends Admin_Controller
{
    ...
    public function getData()
    {
        ...
        $sql = "SELECT *,(@row_number:=@row_number + 1) AS num FROM m_unit, (SELECT @row_number:=0) as temp WHERE 1=1 AND (id_unit LIKE '%" . $string . "%' OR nm_unit LIKE '%" . $string . "%')";
    ...
    public function view($id)
    {
        ...
        $desc = "View Unit Data " . $unit->id_unit . " - " . $unit->nm_unit;
    ...
    public function save()
    {
        ...
        if ($num_unit > 0) {
            ...
            $desc = "Update Unit Data " . $data['id_unit'] . " - " . $data['nm_unit'];
            $device_name = $this->agent->mobile(); // Returns the mobile device name
            if ($this->agent->is_browser()) {
                $device_name = $this->agent->browser(); // Returns the browser name
            } elseif ($this->agent->is_robot()) {
            ...
            $desc = "Insert New Unit Data " . $data['id_unit'] . " - " . $data['nm_unit'];
            $device_name = $this->agent->mobile(); // Returns the mobile device name
            if ($this->agent->is_browser()) {
                $device_name = $this->agent->browser(); // Returns the browser name
            } elseif ($this->agent->is_robot()) {
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            $return    = array(
                'msg'        => 'Failed save data Unit.  Please try again.',
                'status'    => 0
            );
            $keterangan     = "FAILED save data Unit " . $data['id_unit'] . ", Unit name : " . $data['nm_unit'];
            $status         = 1;
            ...
            $return    = array(
                'msg'        => 'Success Save data Unit.',
                'status'    => 1
            );
            $keterangan     = "SUCCESS save data Unit " . $data['id_unit'] . ", Unit name : " . $data['nm_unit'];
            $status         = 1;
            $nm_hak_akses   = $this->addPermission;
            $kode_universal = $data['id_unit'];
            $jumlah         = 1;
    ...
    public function delete()
    {
        ...
        $desc = "Delete Unit Data " . $data['id_unit'] . " - " . $data['nm_unit'];
        $device_name = $this->agent->mobile(); // Returns the mobile device name
        if ($this->agent->is_browser()) {
            $device_name = $this->agent->browser(); // Returns the browser name
        } elseif ($this->agent->is_robot()) {
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            $keterangan     = "Delete data Unit " . $data['id_unit'] . ", Unit : " . $data['nm_packaging'];
            $status         = 1;
            $nm_hak_akses   = $this->addPermission;
            $kode_universal = $data['id_unit'];
            $jumlah         = 1;
