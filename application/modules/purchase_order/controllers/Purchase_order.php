...
class Purchase_order extends Admin_Controller
{
	...
	public function index()
	{
		...
		FROM 
			...
			GROUP BY a.no_po, e.no_pr
			ORDER BY a.no_po, e.no_pr
		DESC")->result();
	...
	public function SaveNew()
	{
		...
		// Untuk PR Departemen
		$this->db->where_in('no_pengajuan', explode(',', $post['so_number']))->update('rutin_non_planning_header', ['po_number' => $code, 'po_date' => date('Y-m-d')]);
	...
	public function PrintH2()
	{
		...
		if($data['header'][0]->tipe == 'pr depart'){
			$data['detail']  = $this->db->query("SELECT a.id as id, a.no_po as no_po, a.id_dt_po as id_dt_po, a.idpr as idpr, a.idmaterial as idmaterial, a.namamaterial as namamaterial, a.description as description, a.hargasatuan as hargasatuan, a.jumlahharga as jumlahharga, a.kode_barang as kode_barang, a.ppn as ppn, a.ppn_persen as ppn_persen, a.harga_total as harga_total, a.tipe as tipe, a.keterangan as keterangan, a.namamaterial as nama, a.kode_barang as code, '1' as konversi, c.code as satuan, '' as satuan_packing, a.qty as qty FROM dt_trans_po a 
			...
			$data['detail']  = $this->db->query("SELECT a.*, b.nama, b.code, b.konversi, c.code as satuan, d.code as satuan_packing FROM dt_trans_po a 
		INNER JOIN new_inventory_4 b ON b.code_lv4 = a.idmaterial 
	...
	public function addPurchaseOrder($filter_status = null)
	{
		...
		$session = $this->session->userdata('app_session');
		$this->template->page_icon('fa fa-users');
		$customer = $this->Pr_model->get_data('customers');
	...
	public function proses()
	{
		...
		$getitemso = $this->db->query("
			SELECT 
				...
				(b.qty_stock) AS avl_stock, 
				c.material_code as code, 
				'' as code1, 
				c.nama as nm_material,
				'' as tipe_pr
			FROM
				material_planning_base_on_produksi_detail a
				LEFT JOIN ms_stock_material b ON b.id_category1 = a.id_material OR b.id_category3 = a.id_material
				LEFT JOIN ms_inventory_category3 c ON c.id_category3 = a.id_material
			WHERE
		...
		$karyawan = $this->db->get_where('employees', ['deleted_by' => null])->result();
		$mata_uang = $this->db->get_where('mata_uang', ['deleted' => null])->result();
