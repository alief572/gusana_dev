...
<script>
	...
	$(document).on('click', '.detailAjust', function(e) {
		...
		$.ajax({
			type: 'POST',
			url: base_url + thisController + '/modal_detail_adjustment/' + $(this).data('kode_trans') + '/' + $(this).data('tanda'),
	...
	$(document).on('change', '#gudang_before', function(e) {
		e.preventDefault();
		$.ajax({
			url: base_url + thisController + '/get_gudang_tujuan',
			cache: false,
			type: "POST",
			data: "id=" + this.value,
			dataType: "json",
	...
	$(document).on('click', '#request', function(e) {
		...
		$.ajax({
			type: 'POST',
			url: base_url + thisController + '/modal_mutasi/' + gudang_before + '/' + gudang_after,
			success: function(data) {
				$("#ModalView2").modal();
				$("#view2").html(data);
	...
	$(document).on('click', '#process_mutasi', function() {
		...
		new swal({
			...
			if (hasil.isConfirmed) {
				...
				$.ajax({
					url: base_url + thisController + '/process_mutasi',
					type: "POST",
					data: formData,
					cache: false,
					dataType: 'json',
	...
	$(document).on('change', '.pack, .stock, .ket', function() {
		...
		$.ajax({
			url: base_url + thisController + '/save_temp_mutasi',
			type: "POST",
			data: {
				"id": id,
				"req_pack": req_pack,
	...
	function get_total_ct() {
		...
		for (a = 1; a < kolom; a++) {
			...
			if (man > max) {
				...
				// new swal({title	: "Error Message!",text	: 'Man minutes melebihi availibility, check again ...',type	: "warning"});
				return false;
			}
	...
	function DataTables(uri_tanda = null) {
		var dataTable = $('#example1').DataTable({
			...
			"ajax": {
				url: base_url + thisController + 'server_side_mutasi_material',
				type: "post",
				data: function(d) {
	...
	function DataTables2(gudang_before = null) {
		var dataTable = $('#example2').DataTable({
			...
			"ajax": {
				url: base_url + thisController + '/server_side_modal_mutasi',
				type: "post",
				data: function(d) {
