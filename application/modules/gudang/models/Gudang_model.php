...
class Gudang_model extends BF_Model
{
	...
	{
		$pusat				  = $this->db->query("SELECT * FROM m_warehouse")->result_array();
	...
	public function get_data_json_mutasi_material()
	{
		...
		foreach ($query->result_array() as $row) {
			...
			$nestedData[]	= "<div align='left'>" . get_name('m_warehouse', 'warehouse_nm', 'id', $row['id_gudang_dari']) . "</div>";
			$nestedData[]	= "<div align='left'>" . get_name('m_warehouse', 'warehouse_nm', 'id', $row['id_gudang_ke']) . "</div>";
	...
	public function process_mutasi()
	{
		...
		foreach ($detail as $val => $valx) {
			...
			if ($req_stock > 0) {
				...
				$ArrHist[$val]['kd_gudang'] 		= get_name('m_warehouse', 'id', 'id', $gudang_before);
				$ArrHist[$val]['id_gudang_dari'] 	= $gudang_before;
				$ArrHist[$val]['kd_gudang_dari'] 	= get_name('m_warehouse', 'id', 'id', $gudang_before);
				$ArrHist[$val]['id_gudang_ke'] 		= $gudang_after;
				$ArrHist[$val]['kd_gudang_ke'] 		= get_name('m_warehouse', 'id', 'id', $gudang_after);
				...
				if (!empty($rest_subgudang)) {
					...
					$ArrHistSub[$val]['kd_gudang'] 			= get_name('m_warehouse', 'id', 'id', $gudang_after);
					$ArrHistSub[$val]['id_gudang_dari'] 	= $gudang_before;
					$ArrHistSub[$val]['kd_gudang_dari'] 	= get_name('m_warehouse', 'id', 'id', $gudang_before);
					$ArrHistSub[$val]['id_gudang_ke'] 		= $gudang_after;
					$ArrHistSub[$val]['kd_gudang_ke'] 		= get_name('m_warehouse', 'id', 'id', $gudang_after);
				...
				if (empty($rest_subgudang)) {
					$sql_mat	= "	SELECT a.* FROM ms_inventory_category3 a WHERE a.id_category3 = '" . $rest_pusat[0]->id_material . "' LIMIT 1";
					// echo $sql_mat; exit;
					$rest_mat	= $this->db->query($sql_mat)->result();
					//update stock sub gudang
					$ArrStockSupIns[$val]['id_material'] 	= $rest_mat[0]->id_category3;
					$ArrStockSupIns[$val]['idmaterial'] 	= $rest_mat[0]->id_category3;
					$ArrStockSupIns[$val]['nm_material'] 	= $rest_mat[0]->nama;
					$ArrStockSupIns[$val]['id_gudang'] 		= $gudang_after;
					$ArrStockSupIns[$val]['kd_gudang'] 		= get_name('m_warehouse', 'id', 'id', $gudang_after);
					...
					$ArrHistSubIns[$val]['id_material'] 	= $rest_mat[0]->id_category3;
					$ArrHistSubIns[$val]['idmaterial'] 		= $rest_mat[0]->id_category3;
					$ArrHistSubIns[$val]['nm_material'] 	= $rest_mat[0]->nama;
					$ArrHistSubIns[$val]['id_gudang'] 		= $gudang_after;
					$ArrHistSubIns[$val]['kd_gudang'] 		= get_name('m_warehouse', 'id', 'id', $gudang_after);
					$ArrHistSubIns[$val]['id_gudang_dari'] 	= $gudang_before;
					$ArrHistSubIns[$val]['kd_gudang_dari'] 	= get_name('m_warehouse', 'id', 'id', $gudang_before);
					$ArrHistSubIns[$val]['id_gudang_ke'] 	= $gudang_after;
					$ArrHistSubIns[$val]['kd_gudang_ke'] 	= get_name('m_warehouse', 'id', 'id', $gudang_after);
		...
		$ArrInsertH = array(
			...
			'kd_gudang_dari' 	=> get_name('m_warehouse', 'id', 'id', $gudang_before),
			'id_gudang_ke' 		=> $gudang_after,
			'kd_gudang_ke' 		=> get_name('m_warehouse', 'id', 'id', $gudang_after),
	...
	public function get_data_json_modal_mutasi()
	{
		...
		foreach ($query->result_array() as $row) {
			...
			$get_packaging_unit = $this->db->query('SELECT b.nm_packaging, c.nm_unit FROM ms_inventory_category3 a LEFT JOIN master_packaging b ON b.id = a.packaging LEFT JOIN m_unit c ON c.id_unit = a.unit')->row();
			...
			$nestedData[]	= "<div align='left'>" . strtoupper($get_packaging_unit->nm_packaging) . "</div>";
			$nestedData[]	= "<div align='right'>" . number_format($row['qty_stock'], 2) . "</div>";
			$nestedData[]	= "<div align='left'>" . strtoupper($get_packaging_unit->nm_unit) . "</div>";
			$nestedData[]	= "<div align='left'>
